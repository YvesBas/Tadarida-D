...
// 21/4/2015
// retir� les param�tres ajout�s r�cemment
...
DetecTreatment::DetecTreatment(Detec *pDet)
{
    ...
    _paramVersion = 1; // pour TadaridaD4 - modifi� par config.ini (via setglobalparameters)
    // � revoir avec Yves
    initVectorParams();
...
void DetecTreatment::initVectorParams()
{
    ...
    } // fin bloc des nouveaux param�tres
}
...
void DetecTreatment::shapesDetects()
{
    ...
    //� for(int j=0;j<FFT_HEIGHT_HALF_MAX;j++) memset(_pointFlagsArray[j],0,ld8);
                            ...
                            if(jy>=_minY && jy <= _maxY)
                            {
                                ...
                                for(int jx=ix-retx;jx<=ix+5;jx++)
                                {
                                    if(jx!=ix || jy!=iy)
                                    {
                                        ...
                                        } // fin 2�me condition sur jx

                                    } // fin 1�re condition sur jx
                                    //� :
...
void DetecTreatment::detectsParameter2()
{
    ...
    for (int icri = 0 ; icri < nbcris ; icri++) //Execute for each call
    {
        ...
        */
        ...
        //_detec->_logText << "Apr�s calcul des hetx, hety"  << endl; //+++
        // ---------------------------------------------------
        ...
        //_detec->_logText << "Apr�s calcul stablr et stabbr"  << endl; //+++
        float enerMaster[2];
...
//        _detec->_logText << "Apr�s calcul des Hei..."  << endl;  //+++
        // ��������������������������������������������������
        if(_imageData)
        {
        ...
        for(int jcrete=0;jcrete<NCRETES;jcrete++)
        {
            ...
            if(jcrete<3)
            {
                ...
                if(_paramVersion>=1)
                {
                    ...
                    //
                    // -----------------------------------------------------------------------------------------
                    // Param�tres de la s�rie Coudes
                    // _detec->_logText << "jcrete=" << jcrete << " s�rie coudes" << endl;
                    int em=3;
                    ...
                    // -----------------------------------------------------------------------------------------
                    // nouveau param�tres (� r�int�grer dans boucles du d�but de la m�thode)
                    if(jcrete==0)
                    {
            ...
            else
            {
                ...
                if(_paramVersion>=1)
                {
                    // calcul du nouveau param�tre SDC
                    oParamCrete[jcrete][SDC]  = 0;
                    oParamCrete[jcrete][SDCR]  = 0;
                    int derdif1=0;
                    int dif1,dif2;
...
void DetecTreatment::saveParameters(const QString& wavFile)
{
    ...
    {
        _detec->_logText  << "ouverture en �criture du fichier " << txtFilePath << " impossible !" << endl;
        return;
    }
